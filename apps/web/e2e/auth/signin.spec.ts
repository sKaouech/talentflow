...
test.describe('Connexion utilisateur', () => {
  ...
  test('devrait afficher la page de connexion correctement', async ({
    page,
  }) => {
    ...
    await expect(
      page.getByText(/connectez-vous à votre compte talentflow/i)
    ).toBeVisible()
  ...
  test("devrait permettre la navigation vers la page d'inscription", async ({
    page,
  }) => {
  ...
  test('devrait rediriger les utilisateurs non authentifiés', async ({
    page,
  }) => {
