...
import { Button, Card, Badge, Input, Avatar, AvatarImage, AvatarFallback } from '@talentflow/ui'
import { 
  Plus, 
  Search, 
  Filter,
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <Card className="p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-slate-600">Total candidats</p>
              <p className="text-3xl font-bold text-slate-900 mt-2">{mockCandidates.length}</p>
            </div>
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-slate-900 mt-2">
                {Math.round(mockCandidates.filter(c => c.dailyRate).reduce((acc, c) => acc + (c.dailyRate || 0), 0) / mockCandidates.filter(c => c.dailyRate).length)}€
              </p>
            </div>
      ...
      <Card className="p-6">
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <div className="flex gap-2 overflow-x-auto">
            ...
            >
              Disponibles ({mockCandidates.filter(c => c.status === 'active').length})
            </Button>
            <Button
            ...
            >
              En mission ({mockCandidates.filter(c => c.status === 'busy').length})
            </Button>
          </div>
        </div>
      </Card>
        ...
        {filteredCandidates.map((candidate) => (
          <Card key={candidate.id} className="p-6 hover:shadow-lg transition-all duration-200">
            <div className="flex items-start justify-between mb-4">
              ...
              <Badge className={statusConfig[candidate.status as keyof typeof statusConfig].color}>
                {statusConfig[candidate.status as keyof typeof statusConfig].label}
              </Badge>
            </div>
